...
int N;
ll ukuran[3005], harga[3005], dp[3005][3], ans = INF;
...
int main(){
	...
	cin >> N;
	FOR(i,0,N) cin >> ukuran[i];
    FOR(i,0,N) cin >> harga[i];

	FOR(i,0,N) FOR(j,0,3){
		if(j > i) break;
		
		if(j == 0) dp[i][j] = harga[i];
		else{
			ll tmp = INF;
			REVN(k,i-1,0){
				if(ukuran[i] > ukuran[k] && dp[k][j-1] != -1) tmp = min(tmp, dp[k][j-1]);
			}

			if(tmp == INF) dp[i][j] = -1;
			else dp[i][j] = tmp+harga[i];
		}
	}
